...
fn cjk_keyword_match_partial() {
    ...
    assert!(score > 0.8, "Expected score > 0.8 for partial CJK match, got {}", score);
...
fn cjk_keyword_match_token_overlap() {
    ...
    assert!(score > 0.0, "Expected positive score for CJK token overlap, got {}", score);
    // "知识管理" tokens: 知, 识, 管, 理
    // "知识库管理系统" contains: 知, 识, 管, 理
    // Should have high overlap
    assert!(score >= 0.75, "Expected high token overlap score, got {}", score);
}
...
fn cjk_keyword_match_mixed_language() {
    ...
    assert!(score > 0.8, "Expected high score for mixed language match, got {}", score);
    
    let score2 = keyword_match_score("memory记忆", "memory记忆存储");
    assert!(score2 > 0.8, "Expected high score for mixed language partial match, got {}", score2);
}
...
fn cjk_keyword_match_no_match() {
    ...
    assert_eq!(score, 0.0, "Expected zero score for no CJK overlap, got {}", score);
}
...
fn cjk_keyword_match_japanese_hiragana() {
    ...
    assert!(score >= 0.0, "Expected valid score for Japanese text, got {}", score);
}
...
fn cjk_keyword_match_chinese_punctuation() {
    ...
    assert!(score > 0.8, "Expected high score for CJK with punctuation, got {}", score);
}
...
fn cjk_keyword_match_traditional_chinese() {
    ...
    assert!(score > 0.8, "Expected high score for Traditional Chinese, got {}", score);
}
...
fn cjk_keyword_match_single_character() {
    ...
    assert!(score > 0.0, "Expected positive score for single CJK character, got {}", score);
}
