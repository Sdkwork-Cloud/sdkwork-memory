...
///
/// **Production:** use [`build_router_with_backend_api`] or
/// [`build_router_with_open_memory_service`] with a concrete `OpenMemoryService` so commercial
/// routes are fully wired. Trait-only mounts return `501` for commercial management endpoints.
