...
execSync("cargo build --release -p sdkwork-memory-api-server", {
  cwd: root,
  stdio: "inherit",
});
...
const releaseBinary = (() => {
  for (const candidate of [
    join(root, "target", "release", "sdkwork-memory-api-server.exe"),
    join(root, "target", "release", "sdkwork-memory-api-server"),
  ]) {
    try {
...
cpSync(releaseBinary, join(stagingDir, "sdkwork-memory-api-server"));
  ...
  `${JSON.stringify(
    {
      ...
      binary: "sdkwork-memory-api-server",
